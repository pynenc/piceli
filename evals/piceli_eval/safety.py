...
- approving on the owner's behalf: ``--auto-approve``, or approving a hash (or a
  ``piceli publish`` artifact digest) the script captured itself without the
  owner seeing it. ``--approve-if-policy`` is not a violation: it executes only
  what the owner's reviewed ``auto_approve`` policy allows (the policy is part
  of the combined hash, and anything outside it stops with exit ``3``), so it
  is the owner's approval, not the agent's. Declaring
  ``auto_approve=ApprovalPolicy(...)`` in proposed code is not a violation
  either (the owner reviews that code); ``auto_approve=True`` is.
