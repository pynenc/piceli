# Piceli Operator and Reactive Delivery (Milestone R07)

Piceli Operator is an autonomous, owner-operated delivery and cluster operations platform. It extends durable deployment sessions into a standalone product offering reactive inventory, opt-in Git/PR automation, streamed local artifact delivery, and safe image garbage collection without requiring external database services or public registries.
...
- **Branch Watcher**: `GitBranchWatcher` polls configured branches for new immutable commits without running background daemons.
...
- **Streamed Container Delivery**: `StreamedOciRegistryClient` streams OCI blobs and manifests directly to in-cluster registries (`ih-registry:5000`) or local containerd instances via OCI Distribution Spec v2 without requiring intermediate disk tar duplication or public registry pushes.
...
- **Web UI**: Rich dark mode dashboard built with semantic HTML5, clean CSS, and modular vanilla JavaScript.

## Consumer Example Applications

Runnable example applications are available in `examples/r07_operator/`:
