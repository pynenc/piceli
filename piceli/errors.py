...
ERRORS: Mapping[str, ErrorCode] = _entries(
    ...
    _E(
        ...
        "Dry run skipped for a placeholder image",
        "A `piceli deploy --plan` release preview was computed before the images were built or delivered. Objects that carry a placeholder image (`pending-build`/`pending-delivery`) are never sent to the cluster, not even as a server dry run, so they are compared literally and listed as `apply`.",
        "Nothing to fix. After delivery, `piceli deploy MODULE:ATTR --plan` computes the real release plan with server dry runs.",
        False,
        "kubernetes",
    ),
    _E(
    ...
    _E(
        ...
        "Preview hash is not an approval",
        "The hash passed to `--approve` is the `preview_hash` of a release preview computed with placeholder images (`pending-build`/`pending-delivery`). A preview shows structure and ownership only and is never approvable.",
        "Approve the `combined_hash` from `piceli deploy MODULE:ATTR --plan` instead. It approves the build, the delivery and a release that adopts, replaces or deletes at most what the preview showed. To review the real release plan first, run `--until deliver`, then `--plan` again.",
        False,
        "pipeline",
    ),
    _E(
        "pipeline-preview-changed",
        "Release plan exceeds the approved preview",
        "The run was approved while its images did not exist, so the approval covered the placeholder preview's ownership outcome. After delivery the real release plan adopts, replaces or deletes an object that the preview did not show (an object appeared, changed owner or was declared since). Nothing was applied; build and delivery are finished.",
        "Run `piceli deploy MODULE:ATTR --plan` again (build and delivery are skipped), review the real release plan and approve its combined hash.",
        False,
        "pipeline",
    ),
    _E(
    ...
    _E(
        ...
        "The registry release of `NodeLoopbackRegistry` was applied but did not become ready (image pull, volume or node problem, or its port is held by a process outside the namespace).",
        "Inspect the registry pod with a read-only tool, fix it, then run `piceli deploy MODULE:ATTR --resume`. A registry that already runs in the namespace is detected at plan time (`pipeline-registry-takeover-required`).",
        True,
        "pipeline",
    ),
    _E(
    ...
    _E(
        ...
        "Mirror not pinned by digest",
        "A `mirror=` entry of `NodeLoopbackRegistry` or `Registry` has no `@sha256:` digest. A tag can move, so only digest-pinned images are copied.",
        "Pin the image, for example `docker.io/library/redis@sha256:<digest>` (find the digest with `docker buildx imagetools inspect IMAGE:TAG`).",
        False,
        "pipeline",
    ),
    _E(
        ...
        "Image mirror failed",
        "Copying a `mirror=` image did not succeed and its receipt carries no more specific code.",
        "Check the source and delivery registries, then continue with `piceli deploy MODULE:ATTR --resume`.",
        True,
        "pipeline",
    ),
    _E(
        "mirror-digest-mismatch",
        "Mirrored image does not match its digest",
        "The source (or delivery) registry served manifest or config bytes whose sha256 is not the pinned digest; nothing unverified is pushed.",
        "Check that the reference names the image you meant and that the registry is trustworthy; do not retry against the same source unchanged.",
        False,
        "artifacts-registry",
    ),
    _E(
        "mirror-manifest-invalid",
        "Mirrored manifest not understood",
        "The pinned digest names a document that is not an OCI or Docker image manifest or index (or one that is too large).",
        "Pin the digest of an image manifest or image index (`docker buildx imagetools inspect IMAGE`).",
        False,
        "artifacts-registry",
    ),
    _E(
        "mirror-platform-unavailable",
        "Image has no manifest for the node platform",
        "The mirrored image index holds no manifest for the registry node's `os/architecture`, or a single-platform image was built for another platform.",
        "Pin an image (or index) that supports the node's platform (see `kubectl get node NODE -o jsonpath='{.status.nodeInfo.architecture}'`).",
        False,
        "artifacts-registry",
    ),
    _E(
        "blob-not-found",
        "Blob not found",
        "The source registry has no blob for a digest its manifest references (404).",
        "Check the reference and the registry; mirror another digest if the registry pruned this one.",
        False,
        "artifacts-registry",
    ),
    _E(
        ...
        "Invalid blob redirect",
        "The registry redirected a blob download to an address that is not an http(s) URL.",
        "Check the registry; a proxy in between may rewrite redirects.",
        False,
        "artifacts-registry",
    ),
    _E(
        ...
        "A blob download was redirected more than five times.",
        "Check the registry and any proxy in between, then retry.",
        True,
        "artifacts-registry",
    ),
    _E(
        "pipeline-registry-takeover-required",
        "Existing registry needs takeover",
        "Another Deployment in the namespace already holds the node-loopback registry's port on its node, or a Deployment with the registry's name exists and is not managed by the registry release; a second registry could never start.",
        'Take the live registry over with `NodeLoopbackRegistry(port=PORT, adopt="NAME")` (its objects and data are kept) or `replace="NAME"` (its Deployment is backed up and recreated), or choose another `port=`/`name=`.',
        False,
        "pipeline",
    ),
    _E(
        "pipeline-registry-incompatible",
        "Existing registry cannot be adopted",
        "`NodeLoopbackRegistry(adopt=NAME)` names a live Deployment that is not a compatible loopback registry: not on the host network, another port or node, data on other storage than the declared `host_path=`/`existing_claim=`, or a selector with match expressions.",
        "Declare matching `port=`, `host_path=` or `existing_claim=`, or use `replace=NAME` to recreate the Deployment (the plan shows whether its data carries over).",
        False,
        "pipeline",
    ),
    _E(
        ...
        "Planning the node-loopback registry could not list the namespace's Deployments or read the registry node (to find a live registry and the node platform).",
        "Check that the kubeconfig user may list Deployments in the namespace and get the node, then plan again.",
        True,
        "pipeline",
    ),
    _E(
    ...
    _E(
        "deploy-ref-invalid",
        "Invalid --ref value",
        "A `--ref` value is not `SOURCE=REV` or `REV` (a source name of [A-Za-z0-9._-], a revision without spaces, ranges or a leading `-`), or it names the same source twice.",
        "Pass `--ref SOURCE=REV` once per source, for example `--ref app=main` or `--ref app=$GITHUB_SHA`.",
        False,
        "pipeline",
    ),
    _E(
        ...
        "`--ref SOURCE=REV` names a source that no build of the pipeline declares in its `inputs.toml`.",
        "Use a source name from the builds' `inputs.toml` (`piceli deploy … --plan --json` lists them under `stages.inputs.sources`).",
        False,
        "pipeline",
    ),
    _E(
        ...
        "Ambiguous --ref",
        "A bare `--ref REV` needs a pipeline whose declared sources are all one repository, and cannot be combined with other `--ref` values; or two builds declare the same source name at different paths.",
        "Name each source: `--ref SOURCE=REV` (repeat the option per source).",
        False,
        "pipeline",
    ),
    _E(
        "deploy-ref-unknown",
        "Unknown revision",
        "The revision given to `--ref` is not a commit in the source's local repository (a typo, or a branch, tag or commit that was not fetched).",
        "Fetch it (`git fetch`) or pass an existing branch, tag or full commit SHA.",
        True,
        "pipeline",
    ),
    _E(
        "deploy-ref-checkout-failed",
        "Checking out the revision failed",
        "`git worktree add` could not check the pinned commit out into a temporary directory (disk space, a checkout filter such as Git LFS without its objects, or file permissions).",
        "Fix the cause git reports on stderr (for example `git lfs fetch`) and run the command again.",
        True,
        "pipeline",
    ),
    _E(
        "deploy-ref-model-differs",
        "Pipeline module differs from the commit",
        "With `--ref` the pipeline module is imported from the working tree, and its Python files (the module and the `.py` files beside it) must equal the pinned commit so the release is exactly that commit; they differ.",
